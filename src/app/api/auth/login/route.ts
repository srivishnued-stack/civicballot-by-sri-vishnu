...
function exact(value:string,expected:string|undefined){if(!expected)return false;const a=Buffer.from(value);const b=Buffer.from(expected);return a.length===b.length&&timingSafeEqual(a,b)}
...
export async function POST(req:Request){
  try{
    ...
    if(body.data.role==="admin"){
      ...
      if(!adminId||!adminPassword)return NextResponse.json({error:"Administrator credentials are not configured in Vercel."},{status:503});
      if(!exact(body.data.identity,adminId)||!exact(body.data.password,adminPassword))return NextResponse.json({error:"Invalid administrator ID or password."},{status:401});
      ...
      const systemEmail=`${adminId.toLowerCase()}@civicballot.local`;
      let [owner]=await db.select().from(users).where(and(eq(users.organizationId,org.id),eq(users.email,systemEmail))).limit(1);
      if(!owner)[owner]=await db.insert(users).values({organizationId:org.id,name:adminId,email:systemEmail,passwordHash:await hash(adminPassword,12),role:"owner"}).returning();
